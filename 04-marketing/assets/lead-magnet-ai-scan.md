# Lead Magnet — "De AI-Scan voor installatie- en servicebedrijven"

Implements LEADS-2 (narrow problem solved completely: *"waar begin ik?"* → self-diagnosis in 10 minutes; the score reveals the bigger problem the Kickstart solves). Voice per `M5-voice-and-tone.md` (re-tuned to approved voice, 2026-07-06). Format: 8–10 page PDF (design in Canva/Gamma) + optional interactive version (Tally/Typeform with auto-score). Delivery: email after opt-in; last page CTA = free 20-min debrief.

**Production note:** the full text is below — paste per section into your design tool. Keep the scoring honest: a high score must genuinely mean "you're fine without me" (trust > leads).

---

## Cover

**Hoeveel uren lekt jouw bedrijf per week?**
De AI-Scan voor installatie- en servicebedrijven: 12 vragen, 10 minuten, jouw score en de 10 automatiseringen met de snelste terugverdientijd.
*Door Olivia Vanmalleghem, AI-integratiecoach voor de vakman-sector*

## Pagina 1 — Waarom deze scan

Je hoort overal dat je iets met AI moet, terwijl je dagen al vol zitten: offertes die te laat vertrekken, avonden vol admin, en een vacature die maar niet ingevuld raakt.

Wat ik in de praktijk zie: de meeste winst zit niet in dure software of een data-afdeling, maar in vijf à tien taken die vandaag mensenuren opeten en dat morgen niet meer hoeven te doen. Deze scan toont je in tien minuten waar die zitten, met de cijfers erbij om het aan jezelf te bewijzen.

Wat niet werkt voor een bedrijf van jouw grootte, staat er niet in.

## Pagina 2–4 — De scan (12 vragen, score 0/1/2 elk)

*Kruis aan: 0 = nee/nooit · 1 = soms/deels · 2 = ja/altijd. Tel op per blok.*

**Blok A — Offertes & verkoop**
1. Vertrekt een offerte gemiddeld binnen de 48u na het plaatsbezoek?
2. Wordt elke gemiste oproep dezelfde dag nog beantwoord (ook tijdens werfuren)?
3. Volg je openstaande offertes systematisch op (niet "als er tijd is")?

**Blok B — Werf & werkbonnen**
4. Staan werkbonnen dezelfde dag digitaal in je systeem (geen papier, geen overtypen)?
5. Vindt je team klanthistoriek (vorige interventies, toestellen, afspraken) in minder dan 2 minuten?
6. Verliest je beste technieker minder dan een halfuur per dag aan administratie?

**Blok C — Facturatie & geld**
7. Vertrekt de factuur binnen de 48u na de werf?
8. Lopen betalingsherinneringen automatisch (niemand "moet er nog achter")?
9. Weet je per klant/werftype wat je marge is, zonder er een weekend voor te rekenen?

**Blok D — Team & kennis**
10. Ligt de kennis van je bedrijf (prijzen, werkwijzen, klanten) ergens anders dan in één iemands hoofd?
11. Gebruikt je team vandaag al AI-tools mét afspraken (niet stiekem, niet verboden)?
12. Heb je dit jaar één werkproces bewust vereenvoudigd of geautomatiseerd?

## Pagina 5 — Jouw score

- **0–8 · Rood: de uren lekken hard.** Reken op 10 à 15 uur per week aan recupereerbare capaciteit. Het goede nieuws is dat bij deze score de snelste winsten voor het rapen liggen, want de terugverdientijd van de eerste automatisering is hier typisch twee à drie maanden.
- **9–16 · Oranje: het fundament staat er, maar de motor sputtert.** Je hebt tools, maar het werk stroomt er niet doorheen. De winst zit in het verbinden en in het herverdelen van taken, niet in nog meer software.
- **17–24 · Groen: sterk bezig.** Jij haalt meer uit een groeigerichte aanpak (nieuwe diensten, onderhoudscontracten, capaciteit naar verkoop) dan uit basisautomatisering. Eerlijk is eerlijk: een Kickstart heb je waarschijnlijk niet nodig.

**De valkuil bij elke score:** losse tools kopen zonder het werk te herverdelen. Een licentie op zich verandert niets, omdat het pas beweegt wanneer wie-doet-wat mee verandert. Dat is het echte probleem achter de score, en precies wat een goede aanpak oplost.

## Pagina 6–7 — De top-10 automatiseringen met de snelste terugverdientijd

*Per stuk: wat het doet · wat het typisch oplevert · wat je nodig hebt.* (Neem de tabel over uit de sectorbibliotheek in `03-clients/sme-home-services/sme-ai-kickstart.md`, ingekort tot één regel per item: gemiste-oproep-sms, spraakmemo-werkbon, offerte-assistent, offerte-opvolging, werkbon→factuur, betalingsherinneringen, mailbox-triage, reviewbeheer, onderhoudscontract-reminders, werfvoorbereidingspakket. Cijfers als vork, bv. "3–7 uur/week bij 2+ techniekers".)

## Pagina 8 — De 3 klassieke fouten

1. **Tool-eerst denken.** Eerst licenties, dan pas nadenken over het werk. Andersom werkt: eerst de uren-lek vinden, dan pas de tool kiezen (vaak kan je huidige software het al).
2. **De mensen vergeten.** De beste automatisering sterft als het team ze niet vertrouwt. Wie ermee moet werken, bouwt mee — vanaf dag één.
3. **Alles tegelijk willen.** Twee automatiseringen die drááien verslaan tien plannen. Kies op terugverdientijd, meet, en bouw dan verder.

## Pagina 9 — Volgende stap (één CTA)

**Gratis debrief van jouw scan: 20 minuten, telefoon of Teams.**
Je krijgt: (1) wat jouw score betekent voor jouw bedrijf, (2) de twee automatiseringen waar ik bij jou zou beginnen en waarom, (3) het eerlijke antwoord of dit iets voor jou is of nog niet.
Geen verkooppraatje, want als het niets voor je is, zeg ik je dat even duidelijk.
**Boek hier: {{CALENDLY-LINK}}**

*Olivia Vanmalleghem helpt installatie- en servicebedrijven tien of meer uur per week terugwinnen met AI en automatisering, zonder extra aanwervingen. linkedin.com/in/olivia-vanmalleghem · {{SITE — optional, later}}*

---

## Regeneration prompt (new niche / language)

```
Using M1 (LEADS-2 rules), M2's new positioning for {{NICHE}}, M5-voice-and-
tone.md (mid-form register, forbidden list), and the top-10 use-case list
from the niche-swap output, rewrite this lead magnet completely:
same structure (cover, why, 12-question self-scan in 4 blocks scored 0/1/2,
score interpretation incl. an honest "you don't need me" tier, top-10 quick-
payback automations, 3 classic mistakes, single debrief CTA). Questions must
make the reader FEEL the hours leaking; language {{NL/FR/EN}}, register
matching how {{NICHE}} owners actually talk. The scan must reveal that the
real problem is work-redistribution, not tools — the bridge to my paid offer.
```
